...
# @app.get("/")응답에 대한 요청을 react로 돌려주기만 하면 끝나므로 return templates는 의미 없다.
...
class HealthResponse(BaseModel):
    ...
    ollama_status: str
    message: str
@app.get("/ai/health") #리액트에서 /ai를 붙이면 이쪽으로
...
class PoemsRequest(BaseModel):
    topic:str
    style:str
@app.post('/ai/poem')#리액트에서 /ai를 붙이면 이쪽으로
...
########################## 채팅 내용을 레디스에 저장 ############################
# 클라이언트로부터 받은 채팅 메시지를 Ollama(비동기 클라이언트)에 전달해 응답을 받고,
# 그 대화 기록을 Redis에 저장한 뒤 AI의 응답을 JSON으로 반환합니다.
# 전체 흐름은 요청 수신 → 모델 호출 → 응답 수집 → 히스토리 업데이트 → Redis 저장 → 응답 반환입니다.
...
######################### 레디스에서 읽어오기 ##############################
#세션 ID로 Redis에 저장된 채팅 기록을 불러와 JSON 배열로 반환합니다.
# 클라이언트가 /ai/chat-history/{session_id}로 요청하면
# 해당 세션의 모든 히스토리를 Redis 리스트에서 읽어와 파이썬 객체로 역직렬화한 뒤 응답합니다.
